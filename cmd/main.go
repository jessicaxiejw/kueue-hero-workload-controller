...
func main() {
	...
	// Controller configuration: defaults < --config YAML < flags. Load
	// registers its flags on flag.CommandLine and parses.
	opts := zap.Options{
		Development: true,
	}
	opts.BindFlags(flag.CommandLine)
	heroCfg, err := config.Load(flag.CommandLine, os.Args[1:])
	if err != nil {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "f3f73dcf.coreweave.com",
	...
	// Teardown nudge: the janitor signals, the drain controller wakes
	// queued heroes immediately.
	nudge := make(chan event.GenericEvent, 16)
	if err := (&drainctrl.Reconciler{
		Client:   mgr.GetClient(),
		Recorder: mgr.GetEventRecorderFor("hero-drain"),
		Cfg:      &heroCfg,
		Nudge:    nudge,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to set up drain controller")
		os.Exit(1)
	}
	if err := (&janitorctrl.Reconciler{
		Client:   mgr.GetClient(),
		Recorder: mgr.GetEventRecorderFor("hero-janitor"),
		Cfg:      &heroCfg,
		Nudge:    nudge,
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to set up janitor controller")
		os.Exit(1)
	}
